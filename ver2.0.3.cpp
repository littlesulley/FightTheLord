...
int CardCombo::getValue() const {
	...
	case CardComboType::TRIPLET1: return 3 * (comboLevel + 1) - (packs[1].level - 7);		//搭单牌的时候取最小的
	case CardComboType::TRIPLET2: return 3 * (comboLevel + 1) - (packs[1].level - 7) * packs[1].count;	//搭单牌时取最小的
	case CardComboType::BOMB: return 4 * (comboLevel + 1);
	case CardComboType::QUADRUPLE2: return 4 * (comboLevel + 1) - (packs[1].level - 7 + packs[2].level - 7);
	case CardComboType::QUADRUPLE4: return 4 * (comboLevel + 1) - 2 * (packs[0].level - 7) - 2 * (packs[2].level - 7);
	...
	case CardComboType::PLANE2: {
			...
			for(int i = 0; i < 2; ++i){
				temp -= (packs[i + 2].level - 7) * packs[i + 2].count;
			}
			return temp;
		}
	...
	case CardComboType::SSHUTTLE2: {
			...
			for(vector<CardPack>::const_iterator i = packs.begin() + 2; i != packs.end(); ++i){
				temp -= (i->level - 7) * i->count;		//搭从牌的时候取尽可能小的
			}
			return temp;
		}
	case CardComboType::SSHUTTLE4: {
			...
			for(vector<CardPack>::const_iterator i = packs.begin() + 2; i != packs.end(); ++i){
				temp -= (i->level - 7) * i->count;
			}
			return temp;
		}
	case CardComboType::ROCKET: return 100;	//这个参数可以调，100显得太大了，小一点试试看
 ...
 * 生成一个map_value
 */
int best_combo_dp(CardCombo& mydeck, int depth){
    int value = -100;
	...
	for(int i = 0; i <= 17; ++i){
		for(CardCombo cur : combos_in_mydeck[dp_order[i]]){    	//the key is how to accomplish the "c : mydeck" foreach process. 
			...
			int temp_value = best_combo_dp(minus_deck, depth + 1);		//recursion
