	...
	CardCombo(CARD_ITERATOR begin, CARD_ITERATOR end)
	{
		...
		case 1: // 只有一类牌
			...
			case 1: //该类牌为 单张 
				...
				if (curr == 2 && packs[1].level == level_joker) //有两张，其中一张为小王，出小王 
				{
					comboType = CardComboType::ROCKET;
					return;
				}
				if (curr >= 5 && findMaxSeq() == curr &&  //有多张且是顺子，就出顺子 
				...
				break;
			case 2: //该类牌为 对子 
				if (curr == 1)  //只有一对对子，直接出 
				{
					comboType = CardComboType::PAIR;
					return;
				}
				if (curr >= 3 && findMaxSeq() == curr && //有连对，出连对 
					packs.rbegin()->level <= MAX_STRAIGHT_LEVEL)
				{
			...
			case 3: //该类牌为三条 
				if (curr == 1)
				{
					comboType = CardComboType::TRIPLET;  //三不带 
					return;
				}
				if (findMaxSeq() == curr &&
					packs.rbegin()->level <= MAX_STRAIGHT_LEVEL) //飞机 
				{
			...
			break;
		case 2: // 有两类牌
			curr = countOfCount[kindOfCountOfCount[1]];   //牌种更大的那一类牌的数量 
			lesser = countOfCount[kindOfCountOfCount[0]]; //牌种更小的那一类牌的数量 
			if (kindOfCountOfCount[1] == 3)  //更大的那一类牌为三条 
			{
				...
				if (kindOfCountOfCount[0] == 1)
				{
					...
					if (findMaxSeq() == curr && lesser == curr &&  //有飞机就出飞机 
						packs.rbegin()->level <= MAX_STRAIGHT_LEVEL)
					{
				...
				{
					// 三带二
					if (curr == 1 && lesser == 1)  //只有一个三条 
					{
						comboType = CardComboType::TRIPLET2;
						return;
					}
					if (findMaxSeq() == curr && lesser == curr &&  // 三带二飞机 
						packs.rbegin()->level <= MAX_STRAIGHT_LEVEL)
					{
			...
			if (kindOfCountOfCount[1] == 4)
			{
				...
				{
					// 四条带两只 * n
					if (curr == 1 && lesser == 2)  //有两个单张 
					{
						comboType = CardComboType::QUADRUPLE2;
						return;
					}
					if (findMaxSeq() == curr && lesser == curr * 2 &&  //航天飞机 
						packs.rbegin()->level <= MAX_STRAIGHT_LEVEL)
					{
						comboType = CardComboType::SSHUTTLE2;
						return;
